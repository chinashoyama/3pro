...
@interface ViewController () <UITableViewDelegate, UITableViewDataSource>
{
    ...
    NSTimer* _countdown_timer;
}
@property(nonatomic,weak)IBOutlet UIView *toastView;
//@property(nonatomic,strong) NSTimer* _countdown_timer;
...
- (void)timerSetUp {
    ...
    self.timerLabel.text = @"00:00:00";
    mTimer = [NSTimer scheduledTimerWithTimeInterval:5.0f
                                              target:self
...
- (void)timeCounter{
    ...
    self.countPicture.text = [NSString stringWithFormat:@"%2d", count_pic];
    ...
    dispatch_async_default(^{
        ...
        dispatch_async_main(^{
            ...
            NSString *_imageName = info.file_id;
...
- (IBAction)imageSizeClicked:(id)sender
{
    BOOL isSmall = (_storageInfo.image_width <= 2048);
    UIAlertActionStyle largeStyle = isSmall ? UIAlertActionStyleDefault : UIAlertActionStyleDestructive;
    UIAlertActionStyle smallStyle = isSmall ? UIAlertActionStyleDestructive : UIAlertActionStyleDefault;
    
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Image Size"
                                                                   message:nil
                                                            preferredStyle:UIAlertControllerStyleActionSheet];
    UIAlertAction* largeAction = [UIAlertAction actionWithTitle:@"5376 x 2688"
                                                          style:largeStyle
                                                        handler:^(UIAlertAction *action) {
                                                            [_httpConnection setImageFormat:5376 height:2688];
                                                            _storageInfo = [_httpConnection getStorageInfo];
                                                            [self appendLog:[NSString stringWithFormat:
                                                                             @"image size changed[result:%lux%lu]",
                                                                             _storageInfo.image_width, _storageInfo.image_height]];
                                                        }];
    UIAlertAction* smallAction = [UIAlertAction actionWithTitle:@"2048 x 1024"
                                                          style:smallStyle
                                                        handler:^(UIAlertAction *action) {
                                                            [_httpConnection setImageFormat:2048 height:1024];
                                                            _storageInfo = [_httpConnection getStorageInfo];
                                                            [self appendLog:[NSString stringWithFormat:
                                                                             @"image size changed[result:%lux%lu]",
                                                                             _storageInfo.image_width, _storageInfo.image_height]];
    ...
    if (CODE_JPEG == o.objectInfo.file_format) {
        ...
        if ([d isKindOfClass:[ImageViewController class]]) {
            ...
            dispatch_async_default(^{
                ...
                NSMutableURLRequest *request = [_httpConnection createExecuteRequest];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    if (indexPath.section==0) {
        ...
        /*
        NSString *_imageName = obj.objectInfo.file_name;
        UIImage *image = [UIImage imageNamed:_imageName];
        //画像を保存する
        UIImageWriteToSavedPhotosAlbum(image, self, NULL, NULL);
         */
